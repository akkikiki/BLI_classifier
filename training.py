...
class Evaluation(object):
  ...
  def maybe_execute(self, epoch, time):
    ...
    if epoch == self._num_epochs:
      thresholds = [t/10 for t in xrange(0, 10, 1)]
  ...
  def _eval_thresholds(self, thresholds, mode):
    prec_1_vals, prec_all_vals = np.zeros(shape=(10)), np.zeros(shape=(10))
    rec_1_vals, rec_all_vals = np.zeros(shape=(10)), np.zeros(shape=(10))
    f1_1_vals, f1_all_vals = np.zeros(shape=(10)), np.zeros(shape=(10))
    predicted_targets = []
